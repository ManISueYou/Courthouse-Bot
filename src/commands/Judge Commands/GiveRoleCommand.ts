...
//Command Description / command to handle
export default class GiveRoleCommand extends Command {
  public constructor() {
    super("giverole", {
      ...
      description: {
        ...
        examples: ["!giverole @Puzzling, Hey?#6626 @Prosecution"],
      },
  ...
  //command function
  public exec(message: Message, { user, role }: { user: GuildMember, role: Role }): Promise<Message> {
    if (!user.manageable) {
      return message.util.send({
        embed: {
          color: 16733013,
          description: `This bot cannot add roles to this member, please have a higher rank drag this bot above the person you're trying to add.\n If you are the owner, just change your roles, yourself.`,
          footer: {
